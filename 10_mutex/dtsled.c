...
static int chrdevled_open(struct inode *inode, struct file *filp)
{
    ...
    //mutex_lock(&schrdev.lock) ;  //在次阻塞的话，线程不会响应信号，ctrl + c无法终止线程
    mutex_lock_interruptible(&schrdev.lock) ; //线程阻塞会响应信号
...
{
    mutex_unlock(&schrdev.lock) ;
    printk("mutex unlock success.\n") ;
    return 0 ;
}
