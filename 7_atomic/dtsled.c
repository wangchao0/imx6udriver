...
static int chrdevled_open(struct inode *inode, struct file *filp)
{
    ...
    if(!atomic_dec_and_test(&schrdev.lock) )
    {
        return -EBUSY ;
    }
    return 0 ;
...
{
    int atomiccount = 0 ;
    atomiccount = atomic_inc_return(&schrdev.lock) ;
    printk("atomic = %d \n", atomiccount) ;
    return 0 ;
}
...
static int chrdev_init(void)
{
   ...
   u32 regdata[14] ;
   const char *str ;
   atomic_set(&schrdev.lock, 1) ;
   lockcount = atomic_read(&schrdev.lock) ;
   printk("atomic = %d \n", lockcount) ;
   //通过路径查找节点
