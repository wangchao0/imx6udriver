...
static int chrdevled_open(struct inode *inode, struct file *filp)
{
    ...
    //down(&schrdev.lock) ;  //在次阻塞的话，线程不会响应信号，ctrl + c无法终止线程
    down_interruptible(&schrdev.lock) ; //线程阻塞会响应信号
    printk("sema down success.\n") ;
    return 0 ;
} 
