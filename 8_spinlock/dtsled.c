...
struct chrdev_s
{
    ...
    bool statflags ;  //表示led资源状态 1：空闲 0：占用
};
...
static int chrdevled_open(struct inode *inode, struct file *filp)
{
    ...
    if(spin_is_locked(&schrdev.lock) | (schrdev.statflags == false))
    {
        printk("spin_is_locked | statflags = false.\n") ;
        return -EBUSY ;
    }
    else
    {
        spin_lock_irqsave(&schrdev.lock, flags) ;
        schrdev.statflags = false ;
        printk("spin_lock_irqsave, statflags = false\n") ;
        spin_unlock_irqrestore(&schrdev.lock, flags) ;
    }
    return 0 ;
} 
...
static int chrdevled_close(struct inode *inode, struct file *filp)
{
        unsigned long flags ;
        spin_lock_irqsave(&schrdev.lock, flags) ;
        schrdev.statflags = true ;
        printk("spin_lock_irqsave, statflags = true\n") ;
        spin_unlock_irqrestore(&schrdev.lock, flags) ;
...
{
   int err, ret, i, lockcount; 
   struct property *prope ;
   u32 regdata[14] ;
   const char *str ;
   schrdev.statflags = true ;
   spin_lock_init(&schrdev.lock) ;
   //通过路径查找节点
