...
struct chrdev_s
{
    ...
    int led_gpio ;           //led gpio申请的gpio编号
    struct timer_list timer ;//定义内核定时器变量timer
};

#define LED_BLINK_TIME  500   //led500ms闪烁一次
...
{
    static u8 status = 0 ;
    struct chrdev_s *dev = &schrdev ;
    status = !status ;
    printk("timerfuncation status = %d.\n", status) ;
    mod_timer(&dev->timer, jiffies+LED_BLINK_TIME) ; //重启定时器
    gpio_set_value(dev->led_gpio, status) ;
}

static void timer_init(void *arg)
{
    struct chrdev_s *dev = &schrdev ;
    printk("timer_init.\n") ;
    init_timer(&schrdev.timer) ;
    schrdev.timer.expires = jiffies+msecs_to_jiffies(LED_BLINK_TIME) ;
    schrdev.timer.data = 0 ;
    schrdev.timer.function = timerfunction ;
    add_timer(&schrdev.timer) ;
    mod_timer(&schrdev.timer, jiffies+msecs_to_jiffies(LED_BLINK_TIME) );
}

static void timer_exit(void *arg)
{
    struct chrdev_s *dev = (struct chrdev_s*)arg ;
    del_timer(&dev->timer) ;
}
...
static void led_switch(u8 status)
{
    switch(status)
    {
        case '0':
           ...
           gpio_set_value(schrdev.led_gpio, 0 ) ;
           break ;
        case '1':
           printk("led_switch to OFF\n") ;
           gpio_set_value(schrdev.led_gpio, 1);
           break ;
        case '2':
           printk("led switch blink,\n") ;
           timer_init(&schrdev) ;
           break ;
        default:
...
static int chrdev_init(void)
{
   ...
   const char *str ; 
   //通过路径查找节点
   schrdev.np = of_find_node_by_path("/gpioled") ;
   if(schrdev.np == NULL )
   {
       printk("find /gpioled node fail.\n");
       return -1 ;
   }
   else
   {
       printk("find /gpioled success.\n" ) ;
   }
  ...
  //获取属性status属性值 
   ...
   schrdev.led_gpio =  of_get_named_gpio(schrdev.np, "led_gpio", 0) ;
   if(schrdev.led_gpio < 0)
   {
   ...
   err = gpio_request(schrdev.led_gpio, "ledgpio" ) ;
   if(err != 0 )
   {
   ...
   err = gpio_direction_output(schrdev.led_gpio, 1) ;
   if(err != 0 )
   {
       printk("gpio direction set fail.\n") ;
   }
//led_init() ;
   led_switch(LED_ON) ;
